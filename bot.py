...
def get_token():
    try:
        ...
        print("An exception occurred while reading the tokens file")
        return None

    try:
        token = tokens['discord_token_dev']
    except KeyError:
        print("Required key not found in tokens.yaml: discord_token_dev")
        return None

    if os.environ.get("PROD") is not None:
        try:
            token = token['discord_token_prod']
        except KeyError:
            print("Required key not found in tokens.yaml: discord_token_prod")
            return None
